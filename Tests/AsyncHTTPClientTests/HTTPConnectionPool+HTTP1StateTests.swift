...
class HTTPConnectionPool_HTTP1StateMachineTests: XCTestCase {
    func testCreatingAndFailingConnections() {
        ...
        for _ in 0..<8 {
            ...
            guard case .scheduleRequestTimeout(_, for: request.id, on: let waiterEL) = action.request else {
                return XCTFail("Unexpected request action")
            }
            XCTAssert(waiterEL === mockRequest.eventLoop)
            ...
            XCTAssertNoThrow(try waiters.wait(mockRequest, id: request.id))
        }
        ...
        // cancel all waiters
        while let waiter = waiters.timeoutRandomWaiter() {
            let waiterCancelAction = state.cancelRequest(waiter)
            XCTAssertEqual(waiterCancelAction.connection, .none)
            XCTAssertEqual(waiterCancelAction.request, .cancelRequestTimeout(waiter))
        }
    ...
    func testConnectionFailureBackoff() {
        ...
        // 3. waiter times out
        let failRequest = state.waiterTimeout(request.id)
    ...
    func testCancelRequestWorks() {
        ...
        XCTAssertEqual(state.waiterTimeout(request.id), .none, "To late timeout is ignored")
    ...
    func testExecuteOnShuttingDownPool() {
        ...
        guard case .executeRequest(request, connection, cancelWaiter: request.id) = connectedAction.request else {
...
//            let elRequired = (0..<10).randomElement().flatMap { $0 == 0 ? true : false }!
            ...
            guard case .scheduleRequestTimeout(_, request.id, on: let waiterEL) = action.request else {
                return XCTFail("Unexpected request action: \(action.request)")
            }
            ...
            XCTAssertNoThrow(try waiters.wait(mockRequest, id: request.id))
            waitersOrder.append(request.id)
        }
        ...
        while let connection = connections.randomLeasedConnection() {
            ...
            case .none:
                ...
                XCTAssertNoThrow(try connections.execute(waiters.get(waiterID, request: mockRequest), on: connection))
...
//            let elRequired = (0..<10).randomElement().flatMap { $0 == 0 ? true : false }!
            ...
            XCTAssert(mockRequest.eventLoop === waiterEL)
            XCTAssertNoThrow(try waiters.wait(mockRequest, id: request.id))
            waitersOrder.append(request.id)
        }
        ...
        while let closedConnection = connections.randomLeasedConnection() {
            ...
            case .createConnection(let newConnectionID, on: let eventLoop):
                ...
                XCTAssertEqual(waiterID, waitersOrder.popFirst())
                XCTAssertNoThrow(try connections.execute(waiters.get(waiterID, request: request.__testOnly_wrapped_request()), on: newConnection))
