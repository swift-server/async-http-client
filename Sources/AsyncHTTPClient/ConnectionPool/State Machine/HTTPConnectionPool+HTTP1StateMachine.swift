    ...
    struct HTTP1StateMachine {
        ...
        mutating func executeRequest(_ request: Request) -> Action {
            ...
            case .shuttingDown, .shutDown:
                ...
                return .init(
                    request: .failRequest(request, HTTPClientError.alreadyShutdown, cancelWaiter: nil),
                    connection: .none
                )
            }
        }
        ...
        mutating func executeRequestOnPreferredEventLoop(_ request: Request, eventLoop: EventLoop) -> Action {
            if let connection = self.connections.leaseConnection(onPreferred: eventLoop) {
                return .init(
                    request: .executeRequest(request, connection, cancelWaiter: nil),
            ...
            if self.connections.startingGeneralPurposeConnections >= self.queue.count {
                // If there are at least as many connections starting as we have waiters, we
                // don't need to create a new connection. we just need to wait.
                return .init(request: requestAction, connection: .none)
            }

            // There are not enough connections starting for the current waiter count. We should
            // create a new one.
        ...
        mutating func failedToCreateNewConnection(_ error: Error, connectionID: Connection.ID) -> Action {
            ...
            switch self.state {
            case .running:
                // We don't care how many waiters we have at this point, we will schedule a retry.
                // More tasks, may appear until the backoff
        ...
        mutating func timeoutWaiter(_ requestID: Request.ID) -> Action {
            // 1. check waiters in queue
            if let request = self.queue.remove(requestID) {
                return .init(
                    request: .failRequest(request, HTTPClientError.getConnectionFromPoolTimeout, cancelWaiter: nil),
                    connection: .none
                )
            }
        ...
        private mutating func nextActionForIdleGeneralPurposeConnection(
            ...
            // check if there are waiting requests in the general purpose queue
            if let request = self.queue.popFirst(for: nil) {
                return .init(
                    request: .executeRequest(request, self.connections.leaseConnection(at: index), cancelWaiter: request.id),
                    connection: .none
                )
            }
        ...
        private mutating func nextActionForIdleEventLoopConnection(
            ...
            //      eventually.
            //
            // If there is no more work, we close eventLoop bound connections. We don't park them.
            return .init(
                request: .none,
